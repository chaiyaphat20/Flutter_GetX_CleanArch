  ...
  Future<ApiResult> callApi({required NetworkModel method})async {
    try {
      ...
      if(httpNetwork.validStatusCode(statusCode)){
        return ApiResult.success(data: body, url: url, headers: headers, statusCode: statusCode);
      }else{
          return ApiResult.error(data: body, url: url, headers: headers, statusCode: statusCode);
      }
    } on HttpException{
      return const ApiResult.failure(networkException:NetworkException.httpException());  //Api.failure
    } on FormatException{
      return const ApiResult.failure(networkException:NetworkException.formatException());  //Api.failure
    }catch (exception) {
